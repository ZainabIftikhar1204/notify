...
const getAppFilters = (req, res, next) => {
  ...
  const { page, limit, sort, ...filter } = validatedData.value; // Destructure to exclude page, limit, and sort
  req.filter = filter;
  next();
};
...
const getEventFilters = (req, res, next) => {
  ...
  const { page, limit, sort, applicationId, ...filter } = validatedData.value; // Destructure to exclude page, limit, and sort
  req.filter = filter;
  next();
};
